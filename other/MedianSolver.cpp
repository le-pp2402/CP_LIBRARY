  ...
  void balance() {
    if (left.size() == right.size() || left.size() == right.size() + 1) {
      if (left.size() && right.size() &&
        ...
        auto u = *(left.rbegin()); auto v = *(right.begin());
        left.erase(u); right.erase(v);
        sumLeft -= u.first; sumRight -= v.first;
        left.insert(v); right.insert(u);
        sumLeft += v.first; sumRight += u.first;
      }
    } else {
      if (left.size() == 0) {
        auto u = *(right.begin());
        right.erase(u); left.insert(u);
        sumRight -= u.first; sumLeft += u.first;
        return;
      }

      if (left.size() > right.size() + 1) {
        auto u = *(left.rbegin()); 
        left.erase(u); right.insert(u);
        sumLeft -= u.first; sumRight += u.first;
      }
    }
  }
   int getMedian() {
