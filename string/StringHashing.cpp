...
i64 mod_add(i64 a, i64 b, i64 m) { a = a % m; b = b % m;
  return (((a + b) % m) + m) % m; }
i64 mod_mul(i64 a, i64 b, i64 m) { a = a % m; b = b % m;
  return (((a * b) % m) + m) % m; }
i64 mod_sub(i64 a, i64 b, i64 m) {
  a = a % m; b = b % m;
  return (((a - b) % m) + m) % m; }
struct Hashing {
  string s; int n; int primes; const i64 B = 31;
  vector<vector<i64>> H, pB, invPB;
  vector<i64> hMOD = {1000000009, 100000007};
  
  Hashing(string a) {
    primes = hMOD.size(); H.resize(primes);
    pB.resize(primes); invPB.resize(primes);
    s = a; n = s.length();
    for (int i = 0; i < primes; i++) {
      pB[i].resize(n + 1); invPB[i].resize(n + 1);
      pB[i][0] = 1;
      for (int j = 1; j <= n; j++) {
  ...
  vector<i64> get(int l, int r) {
    ...
    for (int i = 0; i < primes; i++) {
      i64 val1 = H[i][r]; i64 val2 = l > 0 ? H[i][l - 1] : 0LL;
